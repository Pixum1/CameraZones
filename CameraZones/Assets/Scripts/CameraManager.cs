...
public class CameraManager : MonoBehaviour
{
    ...
    private float camSwitchSpeed = 50;
    ...
    private void Awake() {
        ...
        GetCurrentZone();
        if(currentZone == null)
            currentZone = zoneManager.zones[0];
    }
    ...
    private void AdjustCamSize() {
        if(currentZone.col.bounds.size.x/currentZone.col.bounds.size.y < cam.aspect) {
            sizeThreshold = currentZone.transform.localScale.y / 2f * (currentZone.col.bounds.size.x / currentZone.col.bounds.size.y) / cam.aspect - .025f;
        }
        else {
    ...
    private void GetCurrentZone() {
        foreach (CameraZone zone in zoneManager.zones) {
            if (zone.m_isActive) {
                currentZone = zone;
            }
        }
